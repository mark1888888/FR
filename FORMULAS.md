...
| `investAmt` | 投資（股票/基金） | `portfolioMarketValue()` — 投資組合各持倉的 `現價 × 持有數` 加總（現價缺則回退為成本）|
...
### 4.5 生存天數（Survival Days）— v1.8.6 新增

```
quickDisposable = bankAmt + cashAmt + investAmt
avgMonthlyExp   = Σ 近 3 個月（不含當月）支出 / 3
survivalDays    = quickDisposable / avgMonthlyExp × 30
```

代表「不增加任何收入的情況下，目前能維持現有支出水平多少天」。
...
- **2026-04-17 v1.9.0**：
  - 新增「財務成長」RPG 頁：老鼠賽跑、成就殿堂、性格診斷、人生大事模擬器
  - 老鼠賽跑核心公式：被動收入 ÷ 總支出，≥100% 跳脫勳章
  - 新增 10 個成就追蹤，首次達成自動通知
  - 人生模擬器內建 6 情境 + 5 年淨資產曲線 + 破產月偵測
- **2026-04-17 v1.8.6**：
  - 新增「財務規劃」模組：專案標籤 / 定期收支 / 儲蓄目標 / 稅務預估
  - 收入/支出加 `projectId` 欄位，可貼專案標籤做跨月損益統計
  - 資產總覽健康指標新增「生存天數」卡（快速支配 ÷ 近 3 月平均月支出）
  - 掃描帳單前可先選定歸屬信用卡
  - 台灣綜所稅 5 級距簡易試算
- **2026-04-17 v1.8.5**：
  - 投資資產唯一來源改為 `d.portfolio`（我的投資組合）
  - 移除資產管理的「投資/其他」分頁與 `ACCOUNT_TYPES` 中的 `invest` / `receivable` / `payable` 選項
  - 一次性清除既有 `accounts[type='invest']` 舊資料（console 會印出移除紀錄）
  - 抽取 `fetchPortfolioPrices()` / `portfolioMarketValue(cur)` 兩個共用輔助函式
  - App 啟動時背景預載投資組合股價，資產總覽一開啟就顯示正確市值
- **2026-04-17 v1.8.4**：
